            ...
            <div className="workspace-split" style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: '2rem', alignItems: 'start' }}>
              ...
              <form onSubmit={saveOfflinePrescription} className="glass-panel no-print" style={{ padding: '2rem', display: 'flex', flexDirection: 'column', gap: '1.5rem', boxShadow: 'var(--shadow-md)' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem', position: 'relative' }}>
                  <label style={{ fontSize: '0.8rem', fontWeight: 700, color: 'var(--primary)' }}>🔍 Search & Add from Medicine Database</label>
                  <input 
                    type="text" 
                    placeholder="Type to search preconfigured medicines..."
                    value={medicineSearchQuery}
                    onChange={e => setMedicineSearchQuery(e.target.value)}
                    style={{ padding: '0.5rem 0.75rem', fontSize: '0.85rem' }}
                  />
                  {medicineSearchQuery.trim() !== '' && (
                    <div style={{
                      position: 'absolute',
                      top: '100%',
                      left: 0,
                      ...
                      {/* ALWAYS show the Option to Add New Medicine to DB at the top */}
                      <div 
                        onClick={async () => {
                          const typedName = medicineSearchQuery.trim();
                          const composition = window.prompt(`Enter composition/generic name for "${typedName}" (Optional, e.g. Paracetamol):`, "");
                          if (composition === null) return;
                          const dosage = window.prompt(`Enter default dosage for "${typedName}" (Optional):`, "");
                          ...
                          try {
                            ...
                            if (data.success) {
                              ...
                              const newRow = { name: data.medicine.name, composition: data.medicine.composition || '', dosage: data.medicine.dosage || '', frequency: '' };
                              setOfflineForm(prev => {
                                const isEmptyFirst = prev.medications.length === 1 && prev.medications[0].name === '' && prev.medications[0].dosage === '';
                                return {
                                  ...prev,
                                  medications: isEmptyFirst ? [newRow] : [...prev.medications, newRow]
                                };
                              });
                      ...
                      >
                        ➕ Add "{medicineSearchQuery}" as New Medicine to DB
                      </div>

                      {(() => {
                        const filtered = medicinesList.filter(med => 
                          med.name.toLowerCase().includes(medicineSearchQuery.toLowerCase()) ||
                          (med.composition && med.composition.toLowerCase().includes(medicineSearchQuery.toLowerCase()))
                        );
                        if (filtered.length === 0) {
                          return (
                            <div style={{ padding: '0.5rem 0.75rem', fontSize: '0.85rem', color: 'var(--neutral-body)', fontStyle: 'italic' }}>
                              No matching medicines found in DB.
                            </div>
                          );
                        }
                        return filtered.map(med => (
                          <div 
                            key={med.id}
                            onClick={() => {
                              const newRow = { name: med.name, composition: med.composition || '', dosage: med.dosage || '', frequency: '' };
                              setOfflineForm(prev => {
                                const isEmptyFirst = prev.medications.length === 1 && prev.medications[0].name === '' && prev.medications[0].dosage === '';
                                return {
                                  ...prev,
                          ...
                          >
                            <div style={{ display: 'flex', flexDirection: 'column' }}>
                              <strong style={{ color: 'var(--neutral-dark)' }}>{med.name}</strong>
                              {med.composition && (
                                <span style={{ fontSize: '0.75rem', color: '#b91c1c', fontStyle: 'italic', marginTop: '0.05rem' }}>
                                  ({med.composition})
                                </span>
                              )}
                            </div>
                            <span style={{ fontSize: '0.75rem', color: 'var(--neutral-body)' }}>Default: {med.dosage}</span>
                          </div>
                        ));
                      })()}
                    </div>
